  ...
  <xsl:template match="/rng:grammar | /rng:element">
    ...
    <xsl:call-template name="gatherSchema">
      ...
      <xsl:with-param name="includes"
        select="document(/rng:grammar/rng:include/@href
    ...
    <xsl:choose>
      <xsl:when test="count($schemas) &lt; count($schemas | $includes)">
        ...
        <xsl:call-template name="gatherSchema">
          ...
          <xsl:with-param name="includes"
            select="document($includes/rng:grammar/rng:include/@href
  ...
  <xsl:template name="output">
    ...
    <sch:schema>
      ...
      <xsl:for-each select="$schemas//sch:ns">
        <xsl:if
          test="generate-id(.) = generate-id($schemas//sch:ns[@prefix = current()/@prefix][1])">
          <xsl:copy-of select="."/>
        </xsl:if>
      </xsl:for-each>
